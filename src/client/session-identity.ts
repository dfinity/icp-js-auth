...
export class SessionIdentity extends DelegationIdentity {
  ...
  constructor(options: SessionIdentityOptions) {
    ...
    // Through adopt(), so it is checked like any other credential from outside.
    // This started as the credential the sign-in ceremony had just minted
    // itself, and now also carries one offered by another tab, so the two
    // suppliers do not have the same trust. One that is not for this account
    // and key is dropped rather than refused loudly: sharing is an
    // optimisation, and minting for ourselves is what a tab that hears nothing
    // does anyway.
