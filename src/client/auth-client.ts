...
import { LocalStateStorage, STATE_KEY, type StateStorage } from './state-storage.js';
import {
...
export class AuthClient {
  ...
  async signOut(options: { returnTo?: string } = {}): Promise<void> {
    ...
    if (options.returnTo !== undefined) {
      ...
      // validated `target.href` is what reaches the sink rather than the raw
      // value, so an invalid or cross-origin `returnTo` is ignored and neither an
      // open redirect nor a `javascript:` execution can be built from it.
  ...
  async #hydrate(): Promise<void> {
    ...
    if (!key || !chain) {
      ...
      if (this.#stateStorage.get(STATE_KEY)?.held) await this.#dropSession();
      return;
    }
    ...
    if (this.#stateStorage.get(STATE_KEY) === null) {
      await this.#endSession();
      return;
    }
  ...
  async #persistChain(chain: DelegationChain): Promise<void> {
    ...
    if (earliest !== null) {
      ...
      this.#stateStorage.set(STATE_KEY, {
  ...
  async #endSession(): Promise<void> {
    this.#stateStorage.remove(STATE_KEY);
  ...
  async #dropSession(): Promise<void> {
    this.#stateStorage.discard(STATE_KEY);
    await this.#clearCredentials();
  }
