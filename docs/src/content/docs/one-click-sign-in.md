...
`verified_email` is available under `openid:` but not under `sso:`. "Verified" means Internet Identity established that the user has access to the address, either directly or through a public identity provider it trusts for that claim. An organization's SSO server is controlled by that organization's own administrators, so there is no basis for the same claim, and the attribute is not offered under `sso:` at all.
