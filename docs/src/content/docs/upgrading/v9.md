...
```diff
- new AuthClient({ identityProvider: 'https://id.ai' });
+ new AuthClient({ identityProvider: { authorizeUrl: 'https://id.ai/authorize' } });
```

Both halves default to mainnet Internet Identity, and `canisterId` sits beside `authorizeUrl` for a local or custom deployment. A string or a `URL` throws a `TypeError` naming the new shape rather than being quietly ignored, since a silent fallback here sends calls to the wrong canister.
