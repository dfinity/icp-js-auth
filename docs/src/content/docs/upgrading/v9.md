...
`signed-in` means this origin holds a credential and can act as the user, and `signed-out` means nobody is. The other two are worth handling: `expired` says the session ended but still names whose it was, so you can say "your session ended" rather than showing a bare signed-out page; `signed-in-elsewhere` says somebody is signed in on this domain while this origin holds nothing for them yet, which you only see once state is shared across subdomains.
...
```diff
- new AuthClient({ identityProvider: 'https://my-ii.icp0.io/authorize' });
+ new AuthClient({
+   identityProvider: {
+     authorizeUrl: 'https://my-ii.icp0.io/authorize',
+     canisterId: 'rdmx6-jaaaa-aaaaa-aaadq-cai',
+   },
+ });
```

Both fields are optional and default to mainnet Internet Identity, so an app pointing at mainnet can leave the option out entirely. A custom or local deployment wants both, since where the ceremony is rendered and which canister mints are now separate facts. A string or a `URL` throws a `TypeError` naming the new shape rather than being quietly ignored, because falling back silently would send the mint calls to mainnet.
