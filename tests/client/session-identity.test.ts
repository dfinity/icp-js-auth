...
function harness(
  ...
  return { identity, mint, newKey, onSessionGone, request };
}
...
describe('SessionIdentity', () => {
  ...
  it('adopts a credential obtained elsewhere', async () => {
    const { identity, mint } = harness();
    const key = Ed25519KeyIdentity.generate();
    const chain = await appDelegation(key);

    expect(identity.adopt({ key, chain })).toBe(true);
    expect(identity.getDelegation()).toBe(chain);
    expect(mint).not.toHaveBeenCalled();
  });

  it('refuses to adopt a credential whose pair does not match', async () => {
    const { identity } = harness();
    const key = Ed25519KeyIdentity.generate();
