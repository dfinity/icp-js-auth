...
describe('indexeddb wrapper', () => {
  ...
  it('should support storing a CryptoKeyPair', async () => {
    ...
    const storedKey = await db.get<CryptoKeyPair>('testKey');

    const serialized = {
      privateKey: serializeCryptoKey(storedKey!.privateKey),
      publicKey: serializeCryptoKey(storedKey!.publicKey),
    };
...
function serializeCryptoKey(key: CryptoKey) {
  return {
